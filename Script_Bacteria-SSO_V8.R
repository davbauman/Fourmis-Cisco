...
# One-tailed tests.
# In females, we only consider gyne, q1d, and q9d (Ind), cut., sperma_(liq+env) (Tissues) 
# --> fusion of liquid and envelopes in bursa and spermatheca (and NO BURSA_COP)
# --> q9d instead of q24d
# We removed two outliars
...
data <- subset(data, Ind != "q3d" & Ind != "q24d" & Tissue != "seminal_ves_env" & 
                 Tissue != "bursa_cop_env" & Tissue != "bursa_cop_liq")
data <- droplevels(data, c("q3d", "q24d", "seminal_ves_env", "bursa_cop_env", "bursa_cop_liq"))
data <- data[-c(26, 45), ]
...
row.names(matcomp) <- c("cuticle_Gyne - cuticle_q1d", "cuticle_q1d - cuticle_q9d", 
                        "sperma_Gyne - sperma_q1d", "sperma_q1d - sperma_q9d")
