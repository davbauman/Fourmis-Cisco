...
# One-tailed tests.
# In females, we only consider gyne, q1d, and q24d (Ind), cut., sperma_(liq+env) (Tissues) 
# --> fusion of liquid and envelopes in bursa and spermatheca (and NO BURSA_COP)
# We removed two outliers
...
data <- subset(data, Ind != "q3d" & Ind != "q9d" & Tissue != "bursa_cop_env" & 
                 Tissue != "bursa_cop_liq")
data <- droplevels(data, c("q3d", "q9d", "bursa_cop_env", "bursa_cop_liq"))
data <- data[-c(31, 50), ]
str(data)
...
w1 <- which(sub[, 1] == "seminal_ves_env")
w2 <- which(sub[, 1] == "seminal_ves_liq")
sub$Tissue[c(w1, w2)] <- "sem_ves"
sub$Tissue <- as.factor(sub$Tissue)
sub$Tissue <- factor(sub$Tissue, levels = c("sem_ves", "accessory_gland", "cuticle"))

boxplot(abund ~ Tissue, data = sub)
# Without cuticle:
w3 <- which(sub$Tissue == "cuticle")
sub2 <- sub[-w3, ]
sub2 <- droplevels(sub2, "cuticle")
boxplot(abund ~ Tissue, data = sub2)
...
listdata[[1]] <- subset(sub, Tissue == "sem_ves")
...
matcomp <- matrix(c(1, 2,
                    1, 3,
                    2, 3), ncol = 2, byrow = T)
row.names(matcomp) <- c("sem_ves - accessory_gland", 
                        "sem_ves - cuticle", 
...
sub <- subset(data, Ind != "Male")
sub <- droplevels(sub, c("Male", "accessory_gland", "seminal_ves_liq", "seminal_ves_env"))
...
sub$Tissue <- as.factor(sub$Tissue)
sub$Tissue <- factor(sub$Tissue, levels = c("spermatheca", "cuticle"))
...
listdata <- vector("list", 6)

names(listdata) <- c("cuticle_Gyne", "cuticle_q1d", "cuticle_q24d", "sperma_Gyne", 
                     "sperma_q1d", "sperma_q24d")
...
listdata[[4]] <- subset(sperma, Ind == "Gyne")
listdata[[5]] <- subset(sperma, Ind == "q1d")
listdata[[6]] <- subset(sperma, Ind == "q24d")
...
for (i in 1:nrow(matcomp)) {
  t <- t.perm(listdata[[matcomp[i, 1]]]$abund, listdata[[matcomp[i, 2]]]$abund, 
                     nperm = 9999, silent = TRUE, tail = tail[i])
  matresults[i, 1] <- round(t$t.ref, 2)
  matresults[i, 2] <- round(t$p.perm, 4)
  matresults[i, 3] <- round(sidak(t$p.perm, nrow(matcomp)), 4)
}
...
# Friedman test:
matF <- matrix(c(mean(cuticle$abund[1:5]), mean(sperma$abund[1:9]), mean(cuticle$abund[6:10]),
                 mean(sperma$abund[10:18]), mean(cuticle$abund[11:15]), 
                 mean(sperma$abund[19:27])), ncol = 3)
matF <- matrix(c(median(cuticle$abund[1:5]), median(sperma$abund[1:9]), 
                 median(cuticle$abund[6:10]), median(sperma$abund[10:18]), 
                 median(cuticle$abund[11:15]), median(sperma$abund[19:27])), ncol = 3)
(fried <- friedman.test(matF))
...
datam2 <- datam[-row0, ]
row.names(datam2)
# Removal of q3d and q9d:
datam2 <- datam2[-c(51:88), ]
...
shape <- c(rep(24, 5), rep(21, 2), rep(21, 5), rep(21, 4), rep(24, 5), rep(21, 4), rep(21, 2), 
           rep(21, 5), rep(21, 4), rep(24, 5), rep(21, 2), rep(21, 2), rep(21, 3), rep(21, 2),
           rep(24, 5), rep(21, 5), rep(21, 4), rep(21, 4), rep(21, 3))
# Same colour within a same ind (different tissues differ in the shape):
bg <- c(rep("black", 16), rep("darkgray", 20), rep("green", 14), rep("red", 21))
...
coord <- scores(spe.h.pca, choices = c(1:15), display = "sites")

# II.2. We keep the first 15 PCA axes and use them in an unconstrained Ward clustering:
# *************************************************************************************
...
plot(spe.hwo, hang = -1, xlab = "17 groups", sub = "", ylab = "Height", 
...
hcoplot.no_title_and_legend <- function(tree, diss, k, sep="\n")
{
  require(gclus)
  gr <- cutree(tree, k=k)
  tor <- reorder.hclust(tree, diss)
  par(mar = c(0,2,0,0.5))
  plot(tor, hang=-1,cex=0.7, main="")
  so <- gr[tor$order]
  gro <- numeric(k)
  for (i in 1:k)
  {
...
hcoplot.no_title_and_legend(spe.h.ward, spe.dh, k = k)   # Adapter la valeur de k ici
